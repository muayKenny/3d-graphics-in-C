...
vec3_t barycentric_weights(vec2_t a, vec2_t b, vec2_t c, vec2_t p) {
    ...
    // Alpha is the area of the small parallelogram/triangle PBC divided by the area
    // of the full parallelogram/triangle ABC
    float alpha = (pc.x * pb.y - pc.y * pb.x) / area_parallelogram_abc;

    // Beta is the area of the small parallelogram/triangle APC divided by the area
    // of the full parallelogram/triangle ABC
...
void draw_texel(int x, int y, color_t *texture, vec2_t point_a, vec2_t point_b,
    ...
    if (y1 - y0 != 0) {
        for (int y = y0; y <= y1; y++) {
            ...
            for (int x = x_start; x < x_end; x++) {
                ...
                draw_texel(x, y, texture, point_a, point_b, point_c, u0, v0, u1, v1,
                           u2, v2);
            }
        }
    }
